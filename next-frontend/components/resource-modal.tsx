...
export default function ResourceModal({
  ...
  const selectedResourceType = resourceTypes.find((rt) => rt.id === resourceTypeId)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className={`w-full max-w-md p-6 rounded-lg ${theme === "dark" ? "bg-black" : "bg-white"}`}>
        <div className="flex justify-between items-center mb-6">
          <h2 className={`text-xl font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Add Resource</h2>
          <button onClick={onClose}>
            <X className={`h-5 w-5 ${theme === "dark" ? "text-zinc-400" : "text-gray-500"}`} />
          </button>
        </div>

        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="title" className={`block ${theme === "dark" ? "text-white" : "text-gray-700"} mb-2`}>
              Title
            </label>
            <input
              id="title"
          ...
          <div className="mb-4">
            <label htmlFor="resourceType" className={`block ${theme === "dark" ? "text-white" : "text-gray-700"} mb-2`}>
              Resource Type
            </label>
            <select
              id="resourceType"
              ...
              className={`w-full px-3 py-2 rounded-md ${
                theme === "dark" ? "bg-zinc-900 border-zinc-800 text-white" : "bg-gray-50 border-gray-300 text-gray-900"
              } border`}
          ...
          <div className="mb-6">
            <label htmlFor="total" className={`block ${theme === "dark" ? "text-white" : "text-gray-700"} mb-2`}>
