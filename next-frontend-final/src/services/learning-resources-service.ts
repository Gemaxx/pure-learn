...
export async function getLearningResources(learnerId: string) {
  const res = await fetch(
    `${API_BASE_URL}/api/learners/${learnerId}/LearningResources`
  );
  if (!res.ok) {
...
export async function updateLearningResource(
  ...
  const res = await fetch(
    `${API_BASE_URL}/api/learners/${learnerId}/LearningResources/${id}`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
...
export async function deleteLearningResource(learnerId: string, id: string) {
  const res = await fetch(
    `${API_BASE_URL}/api/learners/${learnerId}/LearningResources/${id}/soft-delete`,
    {
      method: "DELETE",
    }
  );
